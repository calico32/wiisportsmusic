...
export class CommandPlay implements Command {
  ...
  docs = [
    {
      usage: ['play <...playlistUrl>', 'play <...videoUrl>'],
      description:
        'Play a YouTube video, either from a playlist or video URL. The playlist/video must not be private.',
    },
    {
  ...
  async playVideo(cmdArgs: CmdArgs): Promise<void> {
    ...
    queue.voiceConnection?.play(ytdl(video.url)).on('finish', info => {
